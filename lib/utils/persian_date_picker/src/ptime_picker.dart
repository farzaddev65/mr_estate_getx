    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(
            helpText,
            style: TimePickerTheme.of(context).helpTextStyle ?? themeData.textTheme.labelMedium,
          ),
          controls,
        ],
      ),
...
class _HourMinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color backgroundColor = timePickerTheme.hourMinuteColor ??
        MaterialStateColor.resolveWith((Set<MaterialState> states) {
          return states.contains(MaterialState.selected) ? themeData.colorScheme.primary.withOpacity(isDark ? 0.24 : 0.12) : themeData.colorScheme.onSurface.withOpacity(0.12);
        });
    final TextStyle style = timePickerTheme.hourMinuteTextStyle ?? themeData.textTheme.headlineMedium!;
...
class _StringFragment extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TimePickerThemeData timePickerTheme = TimePickerTheme.of(context);
    final TextStyle hourMinuteStyle = timePickerTheme.hourMinuteTextStyle ?? theme.textTheme.headlineMedium!;
...
class _RenderInputPadding extends RenderShiftedBox {
  ...
  bool hitTest(BoxHitTestResult result, {required Offset position}) {
    ...
    if (position.dx < 0.0 || position.dx > math.max(child!.size.width, minSize!.width) || position.dy < 0.0 || position.dy > math.max(child!.size.height, minSize!.height)) {
      return false;
    }
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  double _getThetaForTime(TimeOfDay time) {
    ...
    final double fraction = widget.mode == _TimePickerMode.hour ? (time.hour / hoursFactor) % hoursFactor : (time.minute / TimeOfDay.minutesPerHour) % TimeOfDay.minutesPerHour;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            widget.helpText,
            style: TimePickerTheme.of(context).helpTextStyle ?? theme.textTheme.labelMedium,
          ),
          const SizedBox(height: 16.0),
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
    ...
    final Widget actions = Row(
      children: <Widget>[
        ...
        IconButton(
          ...
          tooltip: _entryMode == PTimePickerEntryMode.dial ? MaterialLocalizations.of(context).inputTimeModeButtonLabel : MaterialLocalizations.of(context).dialModeButtonLabel,
        ),
        Expanded(
          child: Container(
